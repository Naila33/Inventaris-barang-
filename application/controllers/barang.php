...
class Barang extends CI_Controller {
...

    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            // Check if kategori field exists or use id_kategori
            $kategori_value = $this->input->post('kategori');
            ...
            // Add kategori or id_kategori based on table structure
            if ($this->db->field_exists('kategori', 'databarang')) {
                $data['kategori'] = $kategori_value;
            } elseif ($this->db->field_exists('id_kategori', 'databarang')) {
                // If kategori is numeric, treat as id_kategori
                if (is_numeric($kategori_value)) {
                    $data['id_kategori'] = $kategori_value;
                } else {
                    // Try to find kategori by name
                    $this->db->where('nama_kategori', $kategori_value);
                    $kategori_row = $this->db->get('kategoribarang')->row();
                    $data['id_kategori'] = $kategori_row ? $kategori_row->id_kategori : null;
                }
            }
